...
if(!empty($_POST)) {
	if(empty($_POST['tRecurso']) || empty($_POST['lElaboracion']) || empty($_POST['fElaboracion']) || empty($_POST['version']) || empty($_POST['tipoRecurso']) || empty($_POST['idioma']) || empty($_POST['tNavegacion']) || empty($_POST['tGenerador']) || empty($_POST['nEducacion']) || empty($_POST['age']) || empty($_POST['tConexos']) || empty($_POST['oAreas']) || empty($_POST['fEducativa']) || empty($_POST['tArchivo']) || empty($_POST['tRequerido']) || empty($_POST['vProcesador']) || empty($_POST['iPertenece'])) {
		...
		$sql = "INSERT INTO r_educativos(estado, rEducativo, tRecurso, fElaboracion, inicial, pPrimero, pSegundo, pTercero, pCuarto, pQuinto, pSexto, tPrimero, tSegundo, tTercero, tCuarto, tQuinto, inlineCheckbox1, inlineCheckbox2, inlineCheckbox3, inlineCheckbox4, inlineCheckbox5, inlineCheckbox6, inlineCheckbox7, inlineCheckbox8, inlineCheckbox9, inlineCheckbox10, inlineCheckbox11, docente) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		$stmt = $conn->prepare($sql);
		$stmt->execute([$tRecurso, $lElaboracion, $fElaboracion, $version, $tipoRecurso, $idioma, $tNavegacion, $tGenerador, $nEducacion, $age, $tConexos, $oAreas, $fEducativa, $tArchivo, $tRequerido, $vProcesador, $cMinima, $sizeRecurso, $dPerifericos, $aInternet, $sRequerido, $tHerramienta, $Archivo, $Fuente, $Autor, $Institucion, $nApellidos, $iPertenece]);
